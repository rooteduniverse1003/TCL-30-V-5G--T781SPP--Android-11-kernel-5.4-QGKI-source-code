...
#ifdef CONFIG_TCT_UI_TURBO
static inline void uiturbo_dynamic_enqueue_binder(struct binder_thread *thread,
						  struct binder_thread *from)
{
	if (from && test_set_dynamic_uiturbo(from->task) &&
...
static inline void uiturbo_dynamic_dequeue_binder(struct binder_thread *thread)
{
...
#ifdef CONFIG_TCT_UI_TURBO
		if (!oneway)
			uiturbo_dynamic_enqueue_binder(thread, t->from);
#endif
...
#ifdef CONFIG_TCT_UI_TURBO
		uiturbo_dynamic_dequeue_binder(thread);
#endif
		...
		//[TCT-ROM][PERF]Begin Added by jingyuan.wei for freezer on 2019/08/02
                if (binder_freeze_listener && proc->is_frozen) {
                        ...
                        if (is_cur_frozen != proc->is_cur_frozen && !proc->is_processing) {
                                ...
                                binder_debug(BINDER_DEBUG_FREEZE,"%d need %sfreeze pid:%d client:%d line:%d\n",proc->pid,proc->is_cur_frozen?"":"un",proc->pid,freeze_client,__LINE__);
                        }
                }
                //[TCT-ROM][PERF]End Added by jingyuan.wei for freezer on 2019/08/02
...
#ifdef CONFIG_TCT_UI_TURBO
			if (!(t->flags & TF_ONE_WAY))
				uiturbo_dynamic_enqueue_binder(thread, t_from);
#endif
