		...
		unsigned long event, void *data)
{
	...
	printk("DRM event received: %d\n", event);
        printk("pige enter ecm drm_notifier_callback");
	...
	if (blank == DRM_PANEL_BLANK_POWERDOWN && ecm->enable) {
		...
		ecm->reenable = true;
		printk("Disabled ECM for display BLANK\n");
		...
		ecm->reenable = false;
		printk("Enabled ECM for display UNBLANK\n");
	}
        printk("pige enter leave drm_notifier_callback");
	return NOTIFY_DONE;
}
