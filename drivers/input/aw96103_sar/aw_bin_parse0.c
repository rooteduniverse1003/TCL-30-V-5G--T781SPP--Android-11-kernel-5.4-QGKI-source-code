...
*
********************************************************/
int aw_check_sum0(struct aw_bin *bin, int bin_num)
{
	unsigned int i = 0;
...
int aw_check_data_version0(struct aw_bin *bin, int bin_num)
{
	int i = 0;
	DBG("enter\n");
...
int aw_check_register_num_v01(struct aw_bin *bin, int bin_num)
{
...
int aw_check_dsp_reg_num_v01(struct aw_bin *bin, int bin_num)
{
...
int aw_check_soc_app_num_v01(struct aw_bin *bin, int bin_num)
{
...
***
************************/
void aw_get_single_bin_header_1_0_0_0(struct aw_bin *bin)
{
	int i;
	DBG("enter %s\n", __func__);
...
int aw_parse_each_of_multi_bins_1_0_0_0(unsigned int bin_num, int bin_serial_num,
				      struct aw_bin *bin)
{
	int ret = 0;
	...
	ret = aw_parse_bin_header_1_0_0_0(bin);
	return ret;
}

/* Get the number of bins in multi bins, and set a for loop, loop processing each bin data */
int aw_get_multi_bin_header_1_0_0_0(struct aw_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	aw_get_single_bin_header_1_0_0_0(bin);

	for (i = 0; i < bin_num; i++) {
		DBG("aw_bin_parse enter multi bin for is %d\n", i);
		ret = aw_parse_each_of_multi_bins_1_0_0_0(bin_num, i, bin);
		if (ret < 0) {
			return ret;
		}
	}
...
*
********************************************************/
int aw_parse_bin_header_1_0_0_0(struct aw_bin *bin)
{
	int ret = 0;
	...
	case DATA_TYPE_MULTI_BINS:
		...
		ret = aw_get_multi_bin_header_1_0_0_0(bin);
		if (ret < 0) {
			return ret;
		}
		break;
...
int aw_parsing_bin_file0(struct aw_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	for (i = 0; i < bin->all_bin_parse_num; i++) {
		...
		if (bin->header_info[i].bin_data_ver == DATA_VERSION_V1) {
			...
			if (bin->header_info[i].bin_data_type ==
			    DATA_TYPE_REGISTER) {
				ret = aw_check_register_num_v01(bin, i);
				if (ret < 0) {
					DBG_ERR
				...
				/* check dsp reg num */
				   ...
				   DATA_TYPE_DSP_REG) {
				ret = aw_check_dsp_reg_num_v01(bin, i);
				if (ret < 0) {
					DBG_ERR
				...
				/* check soc app num */
				   ...
				   DATA_TYPE_SOC_APP) {
				ret = aw_check_soc_app_num_v01(bin, i);
				if (ret < 0) {
					DBG_ERR
