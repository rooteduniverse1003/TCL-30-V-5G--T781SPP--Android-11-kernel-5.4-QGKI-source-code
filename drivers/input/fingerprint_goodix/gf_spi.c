...
static void nav_event_input(struct gf_dev *gf_dev, gf_nav_event_t nav_event)
{
	...
	case GF_NAV_DOUBLE_CLICK:
		...
		break;
//begin modified by zhikui.li,add fingerprint double key
	case GF_NAV_CLICK_EVENT:
		pr_warn("%s nav click\n", __func__);
		break;

	case GF_NAV_DOUBLE_CLICK_EVENT:
		nav_input = GF_NAV_INPUT_DOUBLE_CLICK;
		pr_warn("%s nav double click\n", __func__);
		break;
//end modified by zhikui.li,add fingerprint double key
    default:
		pr_debug("%s nav up\n", __func__);
        break;
	}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_DIR(cmd) & _IOC_READ)
		//retval = !access_ok(VERIFY_WRITE, (void __user *)arg, _IOC_SIZE(cmd));//lzk_debug
		retval = !access_ok((void __user *)arg, _IOC_SIZE(cmd));//lzk_debug
...
int goodix_drm_notifier_callback(struct notifier_block *self,
				unsigned long event, void *data)
{
	...
	pr_err("goodix_fp event:%lu,blank:%d", event, *blank);
...
#if defined(CONFIG_DRM)
    //gf_dev->notifier.notifier_call = goodix_drm_notifier_callback;
    //msm_drm_register_client(&gf_dev->notifier);
    gf_dev->notifier.notifier_call = goodix_drm_notifier_callback;
/*	if (active_panel) {
	    err = drm_panel_notifier_register(active_panel, &gf_dev->notifier);
	    if (err) {
	        pr_err("[FB]Unable to register goodix finger notifier: %d", err);
	    }
	}
*/
#elif defined(CONFIG_FB)
...
error_input:
	// start, guang,xiao@tcl.com, 20210719, unregister input event
	if (gf_dev->input != NULL) {
		input_free_device(gf_dev->input);
		gf_dev->input = NULL;	
	}
	// end, guang,xiao@tcl.com, 20210719, unregister input event
error_dev:
	if (gf_dev->devt != 0) {
