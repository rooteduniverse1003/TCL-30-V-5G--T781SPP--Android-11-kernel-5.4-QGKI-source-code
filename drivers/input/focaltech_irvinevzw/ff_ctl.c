...
//[TCTNB][FINGERPRINT]Modified bgin by Jiancong.Huang for P11424937 at 2021/7/26
...
#if defined(CONFIG_DRM_PANEL)
    if (active_panel)
        drm_panel_notifier_unregister(active_panel, &g_context->fb_notifier);
#else
    if (msm_drm_unregister_client(&g_context->fb_notifier))
        FF_LOGE("[focaltech-DRM]Error occurred while unregistering fb_notifier.\n");
#endif
#elif defined(CONFIG_FB)
    if (fb_unregister_client(&g_context->fb_notifier))
        FF_LOGE("[focaltech-FB]Error occurred while unregistering fb_notifier.");
#endif
//[TCTNB][FINGERPRINT]Modified end by Jiancong.Huang for P11424937 at 2021/7/26
...
//    err = ff_ctl_enable_power(1);
	...
	//added by guang.xiao for fp devinfo begin
    #if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
        snprintf(FP, 128, "FT9362:FOCALTECH");
    #endif
    //added by guang.xiao for fp devinfo end
...
//[TCTNB][FINGERPRINT]Modified bgin by Jiancong.Huang for P11424937 at 2021/7/26
    ...
    tpState = drm_check_dt(dev_node);
    if (tpState) {
        pr_err("parse drm-panel fail");
    }
    if (active_panel) {
        tpState = drm_panel_notifier_register(active_panel, &g_context->fb_notifier);
        if (tpState)
            FF_LOGE("[Focaltech-DRM]drm_panel_notifier_register fail: %d\n", tpState);
    }
#else
    tpState = msm_drm_register_client(&g_context->fb_notifier);
    if (tpState) {
        FF_LOGE("[Focaltech-DRM]Unable to register fb_notifier: %d\n", tpState);
    }
#endif
#elif defined(CONFIG_FB)
    g_context->fb_notifier.notifier_call = ff_ctl_fb_notifier_callback;
    tpState = fb_register_client(&g_context->fb_notifier);
    if (tpState) {
        FF_LOGE("[Focaltech-FB]Unable to register fb_notifier: %d", tpState);
    }
#endif
//[TCTNB][FINGERPRINT]Modified end by Jiancong.Huang for P11424937 at 2021/7/26
