...
//added by guang.xiao for fp devinfo begin
#if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
extern unsigned char FP[128];
#endif
//added by guang.xiao for fp devinfo end
/*
...
static const char *ff_ctl_get_version(void)
{
    ...
    snprintf(version, FF_DRV_VERSION_LEN, "%s", FF_DRV_VERSION);
#ifdef __FF_SVN_REV
...
#ifdef CONFIG_DRM_NOT
int ff_ctl_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data) {
    int *blank = NULL;
    static ff_ctl_context_t *ff_ctl_dev;
    struct drm_panel_notifier *evdata = data;
    blank = evdata->data;

    if ((event != DRM_PANEL_EVENT_BLANK) || (*blank >2)) {
        //pr_err("event(%lu) *blank=%d do not need process\n", event,*blank);
        return 0;
    }

    printk("'%s' enter. event:%lu, blank:%d", __func__, event, *blank);
    ff_ctl_dev = container_of(self, ff_ctl_context_t, drm_notifier);
    switch (*blank) {
        case DRM_PANEL_BLANK_UNBLANK:
            if (ff_ctl_dev->device_available == 1) {
                ff_ctl_dev->drm_black = 0;
                screen_state[0] = 0;
                if (ff_ctl_dev->async_queue)
                    kill_fasync(&ff_ctl_dev->async_queue, SIGIO, POLL_IN);
            }
            break;
        case DRM_PANEL_BLANK_POWERDOWN:
            if (ff_ctl_dev->device_available == 1) {
                ff_ctl_dev->drm_black = 1;
                screen_state[0] = 1;
                if (ff_ctl_dev->async_queue)
                    kill_fasync(&ff_ctl_dev->async_queue, SIGIO, POLL_IN);
            }
...
static int ff_ctl_fb_notifier_callback(struct notifier_block *nb, unsigned long action, void *data)
{
    ...
    int blank;
    char *uevent_env[2];
	/* If we aren't interested in this event, skip it FB_EVENT_BLANK immediately ... */
    ...
    printk("'%s' blank:%d enter.", __func__, blank);
	...
	//FF_LOGV("'%s' leave.", __func__);
    printk("'%s' blank:%d leave.", __func__, blank);
	return NOTIFY_OK;
}
#endif
...
#ifdef CONFIG_DRM_NOT
extern struct drm_panel *ff_active_panel;
#endif
...
#ifdef CONFIG_DRM_NOT
    if (ff_active_panel) {
        if (drm_panel_notifier_unregister(ff_active_panel, &g_context->drm_notifier))
            pr_err("Error occurred while unregistering FocalTech finger_notifier.");
...
static int ff_ctl_init_driver(void)
{
    ...
    do {
        ...
        #ifdef CONFIG_DRM_NOT
            if (ff_active_panel) {
                g_context->drm_notifier.notifier_call = ff_ctl_drm_notifier_callback;
                err = drm_panel_notifier_register(ff_active_panel, &g_context->drm_notifier);
                if (err) {
                   printk("[FB]Unable to register focaltech finger notifier: %d", err);
                   break;
                   //return (-ENODEV);
                }
            }
    ...
    //added by guang.xiao for fp devinfo begin
    #if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
        snprintf(FP, 128, "FT9391:FOCALTECH");
    #endif
    //added by guang.xiao for fp devinfo end
