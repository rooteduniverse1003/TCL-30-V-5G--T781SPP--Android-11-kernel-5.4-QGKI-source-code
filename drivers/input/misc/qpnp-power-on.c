...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	// kaiying.zhang  for print power key log task-10635864
	printk("Report pwrkey %s event,key_code:%d\n", key_status ?
...
/* Add-start by baiwei.peng for FR-10310081 on 2021/04/07 */
#if IS_ENABLED(CONFIG_INPUT_QPNP_POWER_ON_RECOVERY)
static const char *bootmode = NULL;
...
#endif
/* Add-end by baiwei.peng for FR-10310081 on 2021/04/07 */
...
#if ((defined CONFIG_TCT_GCF) || (defined TARGET_BUILD_ENDURANCE))
	return 0;
#endif
...
#if IS_ENABLED(CONFIG_INPUT_QPNP_POWER_ON_RECOVERY)
	if (bootmode) {
        if (strcmp(bootmode, "recovery") == 0) {
            dev_err(dev, "Recovery mode S3 timer set to 128s" );
	        /* S3 debounce is a SEC_ACCESS register */
		        ...
		        rc = qpnp_pon_masked_write(pon, QPNP_PON_S3_DBC_CTL(pon),
					        QPNP_PON_S3_DBC_DELAY_MASK, ilog2(QPNP_PON_S3_TIMER_SECS_MAX));
		        if (rc)
			        return rc;
