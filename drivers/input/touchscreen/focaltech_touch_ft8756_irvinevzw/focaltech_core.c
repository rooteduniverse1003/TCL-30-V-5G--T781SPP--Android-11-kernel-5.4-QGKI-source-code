...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    tct_fts_palm_report(data, buf[1]);
    if (data->log_level >= 3) {
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    if(buf[1]==0x90){
     input_report_abs(data->input_dev, ABS_MT_POCKET_MODE, 0x90); 
     }else if(buf[1]==0x91){
     input_report_abs(data->input_dev, ABS_MT_POCKET_MODE, 0x91);                          //大面积
     }else if(buf[1]==0x92){
     input_report_abs(data->input_dev, ABS_MT_POCKET_MODE, 0x92);                          //开机抬手负值
     }
...
{
    /*Add begin for irvinevzw power on timing sequence by peisong.cao 11396486*/
    extern unsigned int time_tp_clock;
    /*Add end for irvinevzw power on timing sequence by peisong.cao 11396486*/
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    input_set_abs_params(input_dev, ABS_MT_POCKET_MODE, 0, 0xFF, 0, 0);
#if FTS_REPORT_PRESSURE_EN
...
/*Begin Add for 11396486*/
#ifdef CONFIG_TCT_QGKI_IRVINEVZW
unsigned int fts_rst_gpio;
EXPORT_SYMBOL(fts_rst_gpio);
#endif 
/*End Add for 11396486*/
...
/*Begin Add for 11396486*/
#ifdef CONFIG_TCT_QGKI_IRVINEVZW    
    fts_rst_gpio = data->pdata->reset_gpio;
#endif
/*End Add for 11396486*/
    /* request reset gpio */
...
// End added by lingchen for tp info node on 2020-01-21
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
#if !defined(CONFIG_TCT_LCD_TP_GKI_BREMENTF)
...
//Begin added by lingchen for power consumption on 2021-02-02
    #ifdef CONFIG_TCT_QGKI_IRVINEVZW
    if(!Tp_WakeUp)
    #endif
    if (gpio_is_valid(ts_data->pdata->reset_gpio))
        gpio_direction_output(ts_data->pdata->reset_gpio, 0);
//End added by lingchen for power consumption on 2021-02-02
    FTS_FUNC_EXIT();
    return 0;
}
/*Add begin for irvinevzw power on timing sequence by peisong.cao 11396486*/
unsigned int time_tp_clock;
/*Add end for irvinevzw power on timing sequence by peisong.cao 11396486*/
static int fts_ts_resume(struct device *dev)
{
    struct fts_ts_data *ts_data = fts_data;
...
//added by jiaxin.pi for fp devinfo begin
#if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
extern unsigned char ctpinfo[128];
#endif
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    #if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
        snprintf(ctpinfo, 128, "ft8756:focaltech");
    #endif
