...
*****************************************************************************/
#if !defined(CONFIG_TCT_HOLI_IRVINEVZW)
#define KEY_GESTURE_U                           KEY_U
#else
#define KEY_GESTURE_U                           249
#endif
#define KEY_GESTURE_PALMIN                      552
#define KEY_GESTURE_PALMOUT                     553
...
//add by jiaxin.pi for tp Double click wake up for irvinevzw on 2021-07-20 
#ifdef CONFIG_TCT_QGKI_IRVINEVZW
unsigned Tp_WakeUp = 0;
EXPORT_SYMBOL(Tp_WakeUp);
#endif
//add by jiaxin.pi for tp Double click wake up for irvinevzw on 2021-07-20
...
#ifdef CONFIG_TCT_HOLI_IRVINEVZW
static void create_double_wakeup_enable_node(struct fts_ts_data *ts_data){

     if(!ts_data->gesture_dev)
    {
        if(!ts_data->tct_touch_class)
        {
            ts_data->tct_touch_class = class_create(THIS_MODULE, "tct_touch");
            if (IS_ERR(ts_data->tct_touch_class))
            {
               FTS_ERROR("Failed to create class(tct_touch!\n");
                goto get_info_err;
            }
        }
        ts_data->gesture_dev = device_create(ts_data->tct_touch_class,NULL, 0, NULL, "gesture");
        if (IS_ERR(ts_data->gesture_dev))
        {
            FTS_ERROR("Failed to create device(ts_data->gesture_dev)!\n");
            goto get_dev_err;
        }
        if (device_create_file(ts_data->gesture_dev, &dev_attr_double_wakeup_enable) < 0){
            FTS_ERROR("Failed to create double_wakeup_enable file!\n");
        }
    }
...
get_dev_err:
    class_destroy(ts_data->tct_touch_class);
get_info_err:
    return;
}
#endif
...
#ifdef CONFIG_TCT_HOLI_IRVINEVZW
        create_double_wakeup_enable_node(ts_data);
#endif
