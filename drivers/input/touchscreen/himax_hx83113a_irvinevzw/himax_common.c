...
static int himax_self_test_seq_read(struct seq_file *s, void *v)
{
	...
	if (chip_test_r_flag) {
#if defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT_IRVINE)
		if (g_rslt_data)
...
#elif defined(CONFIG_TOUCHSCREEN_HIMAX_INSPECT_IRVINE) || defined(HX_PALM_REPORT)
...
static int drm_check_dt(struct device_node *np)
{
    ...
    if (count <= 0) {
        E("find drm_panel count(%d) fail", count);
        return -ENODEV;
    }
    for (i = 0; i < count; i++) {
...
#else
    ret = drm_check_dt(ts->dev->of_node);
    if(!ret){
	ts->fb_notif.notifier_call = drm_notifier_callback;
        if(active_panel)
	        drm_panel_notifier_register(active_panel,&ts->fb_notif);
    }else{
        E("drm check dt fail\n");
    }
#endif
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_DEBUG_IRVINE)
	if (himax_debug_init())
...
//end modified by xiongbo.huang for irvine resume on 20200905
#endif
	cancel_delayed_work_sync(&ts->work_att);
	destroy_workqueue(ts->himax_att_wq);
#endif
