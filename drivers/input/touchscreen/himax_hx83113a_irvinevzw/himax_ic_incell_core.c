...
//begin modified by xiongbo.huang for irvine porting on 20200830
    uint8_t *w_data;
//	uint8_t w_data[data_len];
    ...
    w_data = (uint8_t*)kzalloc((sizeof(uint8_t)*data_len), GFP_KERNEL);
    if (w_data == NULL) {
		E("%s Allocate  w_data failed\n", __func__);
		return false;
	}
//end modified by xiongbo.huang for irvine porting on 20200830
...
#ifdef CONFIG_TCT_DEVICEINFO
    snprintf(ctp_module_name, PAGE_SIZE, "HX83113A:TXD:0xD%xC%x:AUC0680105C1",ic_data->vendor_display_cfg_ver,ic_data->vendor_touch_cfg_ver);
#endif
