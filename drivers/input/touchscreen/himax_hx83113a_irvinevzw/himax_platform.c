...
//xiongbo
struct spi_transfer *g_read_xfer;
		...
		struct himax_i2c_platform_data *pdata)
{
	...
	if (ret == 0) {
         ...
         */
       //此处从dts 获取值有问题 所以在驱动直接写死
		pdata->abs_x_min = 0;
		pdata->abs_x_max = 1079;
		pdata->abs_y_min = 0;
		pdata->abs_y_max = 2399;
		I("tct  DT-%s:panel-coords = %d, %d, %d, %d\n", __func__,
				pdata->abs_x_min,
				pdata->abs_x_max,
				pdata->abs_y_min,
				pdata->abs_y_max);
...
//此处从dts 获取值有问题 所以在驱动直接写死
	pdata->screenWidth  = 1079;
	pdata->screenHeight = 2399;
	I(" tct DT-%s:display-coords = (%d, %d)\n", __func__,
			pdata->screenWidth,
			pdata->screenHeight);
		...
		uint8_t *data, uint32_t length, uint8_t toRetry)
{
	...
	g_read_xfer[0].tx_buf = command;
	g_read_xfer[0].len = command_len;
	spi_message_add_tail(&g_read_xfer[0], &message);

	g_read_xfer[1].rx_buf = data;
	g_read_xfer[1].len = length;
	spi_message_add_tail(&g_read_xfer[1], &message);
...
//xiongbo
    memset(rBuffer,0x0,HX_MAX_WRITE_SZ);
    result = himax_spi_read(&spi_format_buf[0], 3, rBuffer, length, toRetry);
    for(i = 0;i < length;i++){
	    data[i] = rBuffer[i];
	}
//xiongbo
...
//xiongbo
	rBuffer = kzalloc(sizeof(uint8_t) * HX_MAX_WRITE_SZ, GFP_KERNEL);
	if (rBuffer == NULL) {
		E("%s: allocate rBuffer failed\n", __func__);
		kfree(gBuffer);
		ret = -ENOMEM;
		goto err_alloc_gbuffer_failed;
	}
//xiongbo
	g_read_xfer = kzalloc(sizeof(struct spi_transfer) * 2, GFP_KERNEL);
	if (g_read_xfer == NULL) {
		E("%s, allocate g_read_xfer fail!\n", __func__);
		ret = -ENOMEM;
		goto err_alloc_read_xfer_failed;
	}
    ...
    #if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
        snprintf(ctpinfo, 128, "hx83113a:himax");
    #endif
	return ret;
