...
extern int himax_ts_check_dt(struct device_node *np);
...
#if defined(CONFIG_DRM_PANEL)
    if (active_panel) {
	    printk("tct drm_panel_notifier_register in himax_fb_register\n");
        ret = drm_panel_notifier_register(active_panel, &ts->fb_notif);
        if (ret)
            printk("[DRM]drm_panel_notifier_register fail: %d\n", ret);
    }
#else
