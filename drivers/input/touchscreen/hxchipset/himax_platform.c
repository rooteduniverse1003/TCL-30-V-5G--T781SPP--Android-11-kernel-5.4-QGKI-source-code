...
#if !defined(CONFIG_TCT_GKI_TRANSFOMERVZW)
		...
		struct himax_i2c_platform_data *pdata)
{
	...
	if (ret == -EPROBE_DEFER) {
		#if !defined(CONFIG_TCT_GKI_TRANSFOMERVZW)
		E("himax_ts_check_dt failed return\n");
		return ret;
		#endif
	}
#endif
...
int himax_ts_check_dt(struct device_node *np)
{
	...
	I("himax_ts_check_dt in\n");
	count = of_count_phandle_with_args(np, "panel", NULL);
    I("himax_ts_check_dt in 000 count= %d\n",count);
	if (count <= 0)
		return 0;
    I("himax_ts_check_dt in 111\n");
	for (i = 0; i < count; i++) {
	I("himax_ts_check_dt in\n");
		node = of_parse_phandle(np, "panel", i);
		I("himax_ts_check_dt in222\n");
		panel = of_drm_find_panel(node);
		I("himax_ts_check_dt in333\n");
		of_node_put(node);
		I("himax_ts_check_dt in4444\n");
		if (!IS_ERR(panel)) {
		    printk("tct find drm_panel successfully");
			active_panel = panel;
		    printk(KERN_ERR "active_panelrtct=%s\n",active_panel);
			return 0;
		}
	}
		...
		unsigned long event, void *data)
{
	...
	I("drm_notifier_callback tct in\n");
...
#if !defined(CONFIG_TCT_GKI_TRANSFOMERVZW)
        snprintf(ctpinfo, 128, "HX83102_1ST:CDOT");
#endif
	return ret;
