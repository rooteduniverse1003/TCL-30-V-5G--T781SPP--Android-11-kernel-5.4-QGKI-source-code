 ...
 * Copyright (C) 2021 TCT, Inc.
 *
 * Author gaoxiang.zou@tcl.com
 *
 * $Revision: 47247 $
...
static int check_dt(struct device_node *np)
{
	...
	struct drm_panel *panel;
        ...
        //printk("nv check_dt tct\n");
	count = of_count_phandle_with_args(np, "panel", NULL);
        //printk("nv check_dt tct count=%d\n",count);
	if (count <= 0)
		return 0;

	for (i = 0; i < count; i++) {
        //printk("111nv check_dt tct\n");
		node = of_parse_phandle(np, "panel", i);
        //printk("222nv check_dt tct\n");
		panel = of_drm_find_panel(node);
        //printk("333nv check_dt tct\n");
		of_node_put(node);
        //printk("444nv check_dt tct\n");
		if (!IS_ERR(panel)) {
			novatek_panel = panel;
        //printk("555nv check_dt tct\n");
			return 0;
		}
	}
...
#endif
//printk("tct nvt_ts_probe\n");
	NVT_LOG("nvt_ts_probe tct !!!!!!!!! start\n");

#ifdef __DRM_PANEL_H__
	if (check_dt(dp)) {
        //printk("6666nv check_dt tct\n");
...
#if IS_ENABLED(CONFIG_TCT_DEVICE_INFO)
        snprintf(ctpinfo, 128, "NT36523W_2nd:xinyuan");
#endif
...
static void nvt_ts_early_suspend(struct early_suspend *h)
{
        //printk("tct suspend start\n");
	nvt_ts_suspend(ts->client, PMSG_SUSPEND);
        //printk("tct suspend end\n");
}
...
static void nvt_ts_late_resume(struct early_suspend *h)
{
        //printk("tct resume start\n");
	nvt_ts_resume(ts->client);
        //printk("tct resume end\n");
}
#endif
...
static int32_t __init nvt_driver_init(void)
{
	...
	NVT_LOG("nvt_driver_init tct start\n");
	//---add i2c driver---
	ret = i2c_add_driver(&nvt_i2c_driver);
        //printk("nvt_driver_init tct ret = %d",ret);
	if (ret) {
	//	//printk("failed to add i2c driver");
		goto err_driver;
	}

	NVT_LOG("nvt_driver_init tct finished\n");
