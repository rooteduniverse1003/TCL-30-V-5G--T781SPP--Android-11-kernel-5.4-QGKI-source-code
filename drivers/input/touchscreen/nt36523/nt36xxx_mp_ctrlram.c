...
//begin add by gaoxiang.zou for task 10693519 
#include <linux/fs.h>
#include <linux/syscalls.h>
#define TCT_RAWDATA_RESULT_PATH "/storage/self/primary/rawdata"
//end add by gaoxiang.zou for task 10693519
...
//begin modify by gaoxiang.zou for task 10693519
/*
...
*/
#define SHORT_TEST_CSV_FILE "/storage/self/primary/rawdata/ShortTest.csv"
#define OPEN_TEST_CSV_FILE "/storage/self/primary/rawdata/OpenTest.csv"
#define FW_RAWDATA_CSV_FILE "/storage/self/primary/rawdata/FWMutualTest.csv"
#define FW_CC_CSV_FILE "/storage/self/primary/rawdata/FWCCTest.csv"
#define NOISE_TEST_CSV_FILE "/storage/self/primary/rawdata/NoiseTest.csv"
#define PEN_FW_RAW_TEST_CSV_FILE "/storage/self/primary/rawdata/PenFWRawTest.csv"
#define PEN_NOISE_TEST_CSV_FILE "/storage/self/primary/rawdata/PenNoiseTest.csv"
//end modify by gaoxiang.zou for task 10693519
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
    ...
    if( (TestResult_Short == 0) &&
        (TestResult_Open == 0) &&
        (((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) && (TestResult_FWMutual == 0)) &&
        (((TestResult_Noise == 0) || (TestResult_Noise == 1)) && (TestResult_FW_DiffMax == 0)))
		{
                nvt_mp_seq_printf(m,"PASS");
                return 0;
        }
...
//begin add by gaoxiang.zou for task 10693519
static int tct_dev_mkdir(char *name, umode_t mode)
{
	int err;
	mm_segment_t fs;
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	NVT_LOG("++\n");
        ...
        //begin add by gaoxiang.zou for task 10693519
        if ((tct_dev_mkdir(TCT_RAWDATA_RESULT_PATH, S_IRUGO | S_IWUSR)) != 0){
              NVT_LOG("Failed to create directory for mp_test\n");
        }
        //end add by gaoxiang.zou for task 10693519
