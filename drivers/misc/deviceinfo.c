...
unsigned char ctpinfo[128] = "NA:NA:NA";
EXPORT_SYMBOL(ctpinfo);
show_function(ctpinfo)
DEVICE_ATTR(ctp, 0444, show_ctpinfo, NULL);
//add begin by yusen.ke.sz for show lcm devics info on 20200514
unsigned char lcdinfo[128] = "NA:NA:NA";
EXPORT_SYMBOL(lcdinfo);
show_function(lcdinfo)
DEVICE_ATTR(lcd, 0444, show_lcdinfo, NULL);
//add end by yusen.ke.sz for show lcm devics info on 20200514
...
unsigned char CamNameB3[128] = "NA:NA:NA";
EXPORT_SYMBOL(CamNameB3);
show_function(CamNameB3);
DEVICE_ATTR(CamNameB3, 0444, show_CamNameB3, NULL);
...
// ning.wei++
unsigned char batteryinfo[128] = "NA:NA:NA";
EXPORT_SYMBOL(batteryinfo);
show_function(batteryinfo)
DEVICE_ATTR(battery, 0444, show_batteryinfo, NULL);
...
unsigned char FP[128] = "NA:NA:NA";
EXPORT_SYMBOL(FP);
show_function(FP);
DEVICE_ATTR(fp, 0444, show_FP, NULL);
...
static struct device_attribute *attr[] = {
    ...
    &dev_attr_CamOTPB2,
    &dev_attr_fp,
    &dev_attr_lcd,//add begin by yusen.ke.sz for show lcm devics info on 20200514
    &dev_attr_battery, // ning.wei++ for battery infomation
    NULL,
};
