...
static struct power_supply_attr power_supply_attrs[] = {
	...
	POWER_SUPPLY_ATTR(RESISTANCE_ID), //Added by baiwei.peng for battery id node on 2021/02/10
	POWER_SUPPLY_ATTR(INPUT_SUSPEND), // add by weijun for tct_diag input_suspend
#endif

#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW)  || defined(CONFIG_TCT_HOLI_IRVINEVZW)
	POWER_SUPPLY_ATTR(QUIET_TEMP),//add by zhikui.li charging current limited by quiet thermal,so print it in log
#endif
#endif
