...
/* Modified-start by weijun to charing current on 2021/02/01 */
#if !defined(CONFIG_TCT_CHG_COMMON)
	target_icl = get_adapter_icl_based_ilim(chip);
	ilim = (target_icl > 0) ? min(ilim, target_icl) : ilim;
#endif
/* Modified-end by weijun to charing current on 2021/02/01 */
...
/* Add-start by weijun to charing current on 2021/02/01 */
#if defined(CONFIG_TCT_CHG_COMMON)
...
#endif
/* Add-end by weijun to charing current on 2021/02/01 */
...
/* Start modified by jin.wang to avoid err log for user sw without debugfs */
...
static void qcom_batt_create_debugfs(struct pl_data *chip) {}
#endif
/* End modified by jin.wang to avoid err log for user sw without debugfs */
