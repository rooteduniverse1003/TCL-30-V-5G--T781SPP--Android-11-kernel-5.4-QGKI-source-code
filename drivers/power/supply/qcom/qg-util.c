...
/*start to add by weijun for Battery temp compensate*/
...
#endif
#endif
/*end to add by weijun for Battery temp compensate*/
...
/*start to add by weijun for Battery temp compensate*/
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
	present = is_input_present(chip);

	// pr_err("1-batt_temp = %d present=%d\n", *temp, present);
	if (present == true) {
		if (*temp >= 550) {
			comp_temp = 0;
		} else if (*temp > 350) {
	...
	// pr_err("2-batt_temp = %d present=%d\n", *temp, present);
#endif
#endif
/*end to add by weijun for Battery temp compensate*/
