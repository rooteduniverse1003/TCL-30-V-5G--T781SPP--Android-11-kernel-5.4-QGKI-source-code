		...
		int *val2, long mask)
{
	...
	*val1 = 0;
	...
	if (rc < 0) {
#if defined(CONFIG_TCT_CHG_COMMON)
		pr_debug("Couldn't read IIO channel %d, rc = %d\n",
			chan->channel, rc);
#else
			       ...
			       union power_supply_propval *pval)
{
	...
	if (psp == POWER_SUPPLY_PROP_BATTERY_TYPE)
...
static int qg_determine_pon_soc(struct qpnp_qg *chip)
{
	...
	if (rc < 0) {
		pr_err("Failed to read vbatt at PON rc=%d\n", rc);
	}
#endif
...
#if defined(CONFIG_TCT_CHG_COMMON)
	pr_err("PON:%d,%d,%d,%d,%d,%ld,%d,%d,%d\n",
			shutdown[SDAM_VALID], shutdown[SDAM_SOC],
			shutdown[SDAM_OCV_UV], shutdown[SDAM_TIME_SEC],
...
#if defined(CONFIG_TCT_CHG_COMMON)
	chip->dt.tt_enabled = of_property_read_bool(node, "qcom,tt-enabled");
#endif
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	rc = get_rtc_time(&rtc_sec);
	if (rc < 0) {
		pr_err("failed to register qpnp-qg, deferring probe\n");
		return -EPROBE_DEFER;
	}
#endif
