...
#if defined(CONFIG_TCT_CHG_COMMON)
static ssize_t thermal_disable_show(struct device *dev, struct device_attribute
				*attr, char *buf)
{
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static ssize_t thermal_disable_store(struct device *dev, struct device_attribute
				 *attr, const char *buf, size_t count)
{
	int val;
	struct smb5 *chip = dev_get_drvdata(dev);
	...
	chg->thermal_disable  = val;

	return count;
}
static DEVICE_ATTR_RW(thermal_disable);

/* Begin modified by bitao.xiong for task-10700744 on 2021-02-03 */
#if defined(DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY)
static int fixtemp = 1;
#else
static int fixtemp = 0;
#endif
/* End modified by bitao.xiong for task-10700744 on 2021-02-03 */
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	if (chg->chg_param.hvdcp3_max_icl_ua <= 0)
		chg->chg_param.hvdcp3_max_icl_ua = HVDCP3_CURRENT_UA;
#else
	if (chg->chg_param.hvdcp3_max_icl_ua <= 0)
...
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
    of_property_read_string(node, "qcom,tzone_name",&chg->tzone_name);
#endif
#endif
	...
	if (chg->chg_param.hvdcp2_max_icl_ua <= 0)
		chg->chg_param.hvdcp2_max_icl_ua = HVDCP_CURRENT_UA;
#else
	if (chg->chg_param.hvdcp2_max_icl_ua <= 0)
...
/*start to add by weijun for FR:10247502/10248642/10248631*/
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
	POWER_SUPPLY_PROP_CHG_REAL_TYPE,
#endif
#endif
/*end to add by weijun for FR:10247502/10248642/10248631*/

#if defined(CONFIG_TCT_POGO)
	POWER_SUPPLY_PROP_DOCK_CHG,
#endif
...
/*start to add by weijun for FR:10247502/10248642/10248631*/
#if defined(CONFIG_TCT_CHG_COMMON) \
	&& (defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) \
		|| defined(CONFIG_TCT_HOLI_IRVINEVZW))
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		rc = smblib_get_icl_current(chg, &val->intval);
		break;
	case POWER_SUPPLY_PROP_CHG_REAL_TYPE:
...
#if defined(CONFIG_TCT_POGO)
	case POWER_SUPPLY_PROP_DOCK_CHG:
		pr_debug("is_dock_chg=%d\n", chg->is_dock_chg);
		if (chg->is_dock_chg) {
			val->intval = 1;
		} else {
			val->intval = 0;
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_RESISTANCE_ID, //Added by baiwei.peng for battery id node on 2021/02/10
	POWER_SUPPLY_PROP_INPUT_SUSPEND, // add by weijun for tct_diag input_suspend
#endif

#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
    POWER_SUPPLY_PROP_QUIET_TEMP, //add by zhikui.li charging current limited by quiet thermal,so print it in log
#endif
#endif
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		pval->intval = safety_timer_en;
		break;
/* Add-start by baiwei.peng for battery id node on 2021/02/10 */
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
        rc = smblib_get_prop_from_bms(chg,
				SMB5_QG_RESISTANCE_ID, &pval->intval);
        break;
/* Add-end by baiwei.peng for battery id node on 2021/02/10 */
#endif
...
/* Begin added by bitao.xiong for task-10699316 on 2021-02-03 */
	...
	case POWER_SUPPLY_PROP_ADAPTER_TYPE:
		pval->intval = chg->real_adapter_type_done ? chg->real_adapter_type : 0;
		break;
#endif
#endif
/* End added by bitao.xiong for task-10699316 on 2021-02-03 */

//Add-begin by zhikui.li for report quiet temperature
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
	case POWER_SUPPLY_PROP_QUIET_TEMP:
	{
		int quiet_temp = 0;
		struct thermal_zone_device *thermal_dev;

		thermal_dev = thermal_zone_get_zone_by_name(chg->tzone_name);
		if (!IS_ERR_OR_NULL(thermal_dev)) {
			thermal_zone_get_temp(thermal_dev, &quiet_temp);
			pval->intval = quiet_temp/100;
		}
		else {
			pval->intval = 0;
		}
...
#endif
#endif
//Add-end by zhikui.li for report quiet temperature
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		rc = smblib_masked_write(chg,
				...
				CHGR_SAFETY_TIMER_ENABLE_MASK,
				(val->intval > 0 ? CHG_SAFETY_TIMER_ENABLE :
					CHG_SAFETY_TIMER_DISABLE));
		if (!rc) {
			safety_timer_en = (val->intval > 0 ? 1 : 0);
			pr_err("set safety timer %d done\n", safety_timer_en);
		} else {
			pr_err("set safety timer failed %d\n", rc);
		}
		break;
#endif
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	if (chg->pd_not_supported
		&& (value & TYPEC_LEGACY_CABLE_STATUS_BIT)) {
#else
...
#if defined(CONFIG_TCT_CHG_COMMON)
	smblib_hvdcp_detect_enable(chg, !chg->hvdcp_disable);
#else
...
/* Modify-start by baiwei.peng to disable temperature behaviour on 2021/03/17 */
#if defined(DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY) || \
    defined(TARGET_BUILD_MMITEST) || defined(CONFIG_TCT_GCF)
    rc = smblib_masked_write(chg, JEITA_EN_CFG_REG, 0xFF, 0);
    if (rc < 0) {
        dev_err(chg->dev, "Couldn't configure s/w jeita now, rc=%d\n",
            rc);
        return rc;
    }
#else
...
#endif
/* Modify-end by baiwei.peng to disable temperature behaviour on 2021/03/17 */
