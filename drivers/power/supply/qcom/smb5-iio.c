...
/* Add-start by weijun for charing current on 2021/03/17 */
#if defined(CONFIG_TCT_CHG_COMMON)
...
#endif
#endif
/* Add-end by weijun for charing current on 2021/03/17 */
...
#if defined(CONFIG_TCT_CHG_COMMON)
		pr_err_ratelimited("fcc_main_val:%d,%d, rc=%d\n",
				val, offset_ua, rc);
#endif
	...
	case PSY_IIO_DP_DM:
#if defined(CONFIG_TCT_CHG_COMMON)
		if (val == QTI_POWER_SUPPLY_DP_DM_FORCE_12V) {
			return -EINVAL;
		 ...
		 const char *const *channel_map, int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (IS_ERR(iio_ch_ext[i])) {
			...
			if (rc != -EPROBE_DEFER)
#if defined(CONFIG_TCT_CHG_COMMON)
				dev_dbg(dev, "%s channel unavailable, %d\n",
						channel_map[i], rc);
#else
