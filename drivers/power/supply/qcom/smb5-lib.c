...
//start to add by weijun for dock_pin charge task-10658685
#if defined(CONFIG_TCT_POGO)
#include "pogo_davinci.h"
#endif

#if defined(CONFIG_TCT_CHG_COMMON)
...
#endif
#endif
//end to add by weijun for dock_pin charge task-10658685
...
//start to modify by weijun for dock_pin charge task-10658685
#if defined(CONFIG_TCT_CHG_COMMON)
		icl_override = ICL_OVERRIDE_BIT;
#else
		icl_override = 0;
#endif
//end to modify by weijun for dock_pin charge task-10658685
...
/*start to add by weijun for enable AFP*/
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
	rc = smblib_masked_write(chg, JEITA_EN_CFG_REG, 0xc0, 0xc0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure AFP rc=%d\n",
				rc);
		return rc;
	}
#endif
#endif
/*end to add by weijun for enable AFP*/
...
#if defined(CONFIG_TCT_CHG_COMMON)
	smblib_err(chg, "read thermal:%d,%d,%d,%d, status:%d\n",
...
#if defined(CONFIG_TCT_CHG_COMMON)
	/*
	* Avoid storm of isr when too hot, which will bring more power
	* and more higher temp
	*/
	rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG,
			SNARL_WDOG_TIMEOUT_MASK, SNARL_WDOG_TMOUT_8S);
#else
	rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG,
...
#if defined(CONFIG_GKI_TCT_CHG_COMMON) \
	|| defined(CONFIG_TCT_CHG_COMMON)
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB)
#else
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	/* support for special typec cable */
#if defined(CONFIG_TCT_CHG_COMMON)
...
#if defined(CONFIG_TCT_CHG_COMMON)
		vote(chg->usb_icl_votable, HW_LIMIT_VOTER, false, 0);
#endif
				...
				int *total_current_ua)
{
...
#if defined(CONFIG_TCT_CHG_COMMON)
	/* QC 2.0/3.0 adapter has diff ICL due to CP diff support */
	...
	switch (apsd_result->bit) {
		case CDP_CHARGER_BIT:
			*total_current_ua = CDP_CURRENT_UA;
			break;
		case DCP_CHARGER_BIT:
		case OCP_CHARGER_BIT:
			*total_current_ua = DCP_CURRENT_UA;
			break;
...
/* Begin added by bitao.xiong for task-10699316 on 2021-02-03 */
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_BREMENTF) || defined(CONFIG_TCT_HOLI_BREMENVISIBLE)
#define ADAPTER_TYPE_DELAY_MS 3000
#endif
#endif
/* End added by bitao.xiong for task-10699316 on 2021-02-03 */
...
static void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (rc < 0)
			smblib_err(chg, "Couldn't clear reg0x1380 bit7 rc=%d\n",
					rc);
#endif
...
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_BREMENTF) || defined(CONFIG_TCT_HOLI_BREMENVISIBLE)
		schedule_delayed_work(&chg->adapter_type_work,
					msecs_to_jiffies(ADAPTER_TYPE_DELAY_MS));
#endif
#endif
/* End added by bitao.xiong for task-10699316 on 2021-02-03 */
	} else {
...
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_BREMENTF) || defined(CONFIG_TCT_HOLI_BREMENVISIBLE)
		cancel_delayed_work_sync(&chg->adapter_type_work);
		chg->real_adapter_type = 0;
 		chg->real_adapter_type_done = false;
#endif
#endif
/* End added by bitao.xiong for task-10699316 on 2021-02-03 */
...
#if defined(CONFIG_TCT_CHG_COMMON)
static void smblib_handle_slow_plugin_timeout(
	struct smb_charger *chg, bool rising) {}
#else
					      ...
					      bool rising)
{
	...
	if (apsd_result->bit & QC_3P0_BIT) {
		...
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER,
				true, chg->chg_param.hvdcp3_max_icl_ua);
#endif
					      ...
					      bool rising, bool qc_charger)
{
	...
	if (rising) {
		...
		if (qc_charger) {
#if defined(CONFIG_TCT_CHG_COMMON)
			hvdcp_ua = chg->chg_param.hvdcp2_max_icl_ua;
#else
...
#if defined(CONFIG_TCT_CHG_COMMON)
static void smblib_handle_hvdcp_detect_done(
	struct smb_charger *chg, bool rising) {}
#else
...
//start to add by weijun for dock_pin charge task-10658685
#if defined(CONFIG_TCT_POGO)
	set_current_limit_for_pogo_charger(false);
	/*1 - usb, 2 - dock, 3 - usb+dock*/
	dock_pin = get_dock_pin_status();
	//pr_err("%s:dock_pin=%d val=%d\n", __func__, dock_pin, val);
	if ((dock_pin & POGO_DOCK_CHARGER)
			&& !(dock_pin & POGO_USB_CHARGER)) {
		chg->is_dock_chg = true;
		val = QTI_POWER_SUPPLY_TYPE_USB_FLOAT;
		vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
	} else {
		chg->is_dock_chg = false;
...
//start to modify by weijun for dock_pin charge task-10658685
		...
		vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
#elif defined(CONFIG_TCT_CHG_COMMON)
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
					SDP_CURRENT_UA);
		vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
#else
...
/*start to add by weijun for charger type detect error*/
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_TRANSFORMERVZW) || defined(CONFIG_TCT_HOLI_IRVINEVZW)
	chg->uusb_apsd_rerun_done = false;
#endif
#endif
/*end to add by weijun for charger type detect error*/
...
//start to add by weijun for dock_pin charge task-10658685
#if defined(CONFIG_TCT_POGO)
	schedule_delayed_work(&chg->dock_otg_work,
...
//start to add by weijun for dock_pin charge task-10658685
	...
	if (dock_pin & POGO_DOCK_CHARGER) {
		rc = vote(chg->usb_icl_votable,
			SW_ICL_MAX_VOTER, true, DOCK_CURRENT_UA);
		if (rc < 0)
			return rc;
	}
#endif
...
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_BREMENTF) || defined(CONFIG_TCT_HOLI_BREMENVISIBLE)
#define CHARGE_IS_AC		BIT(0)
#define UN_COMPLICAN_BC1P2 		BIT(1)
#define LOW_RATE_ADAPTER BIT(7)
static void smblib_update_adpater_type(struct smb_charger *chg)
{
	int rc;
	union power_supply_propval val = {0, };
	const struct apsd_result *apsd_result = smblib_get_apsd_result(chg);
	int input_current_threshold = 2000000 * 75 / 100; //unit:uA
	int soc = 0, soc_threshold = 80;
	int adapter_type = 0;
	int input_current_settled = 0;
	int system_temp_level = 0;
	bool chg_type_is_fast = false;
	bool is_ac = false;

	if (apsd_result->bit == FLOAT_CHARGER_BIT ||
			apsd_result->bit == OCP_CHARGER_BIT)
		adapter_type |= UN_COMPLICAN_BC1P2;
	else if (apsd_result->bit == DCP_CHARGER_BIT) {
		is_ac = true;
		adapter_type |= CHARGE_IS_AC;
	}

	rc = smblib_get_prop_batt_health(chg, &val);
	if (rc == 0 && is_ac && val.intval == POWER_SUPPLY_HEALTH_GOOD) {
		rc = smblib_get_prop_batt_charge_type(chg, &val);
		if (rc == 0 && POWER_SUPPLY_CHARGE_TYPE_FAST == val.intval)
			chg_type_is_fast = true;
		rc = smblib_get_prop_from_bms(chg, SMB5_QG_REAL_CAPACITY, &val.intval);
		if (!rc)
			soc = val.intval;
		rc = smblib_get_prop_input_current_settled(chg, &val);
		if (!rc)
			input_current_settled = val.intval;

		rc = smblib_get_prop_system_temp_level(chg, &val);
		if (!rc)
			system_temp_level = val.intval;
		smblib_dbg(chg, PR_MISC, "input_current=%d, soc=%d, system_temp_level=%d\n",
					input_current_settled, soc, system_temp_level);
		if (chg_type_is_fast && system_temp_level < 1) {
			if (input_current_settled < input_current_threshold && soc < soc_threshold) {
				adapter_type |= LOW_RATE_ADAPTER;
			}
		}
	}

	chg->real_adapter_type = adapter_type;
}

static void smblib_adapter_type_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						adapter_type_work.work);
	static int check_count;

	smblib_update_usb_type(chg);
	smblib_update_adpater_type(chg);
	smblib_dbg(chg, PR_MISC, "typec_mode:%d,real adapter type:%d\n",
			chg->typec_mode, chg->real_adapter_type);

	if (chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3 ||
			chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP ||
			chg->pd_active || check_count >= 2 ||
			((chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_FLOAT) &&
				(chg->typec_mode == QTI_POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER))) {
		if(chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3 ||
			chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_HVDCP ||
			chg->pd_active)
			chg->real_adapter_type = CHARGE_IS_AC; //force set AC,ignore LOW_RATE_ADAPTER

		smblib_dbg(chg, PR_MISC, "xbt real_adapter_type = %d\n",
				chg->real_adapter_type);
		chg->real_adapter_type_done = true;
		check_count = 0;
	}
...
//start to add by weijun for dock_pin charge task-10658685
#if defined(CONFIG_TCT_POGO)
static void smblib_dock_otg_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						dock_otg_work.work);
	u8 stat;
	...
	/*1 - usb, 2 - dock, 3 - usb+dock*/
	dock_pin = get_dock_pin_status();
	pr_err("%s:dock_pin=%d, TYPE_C_MISC_STATUS_REG = 0x%02x\n", __func__, dock_pin, stat);
	if ((dock_pin & POGO_DOCK_CHARGER) &&
			(stat & TYPEC_WATER_DETECTION_STATUS_BIT)) {
		pogo_set_otg_detect(1);
	} else {
		pogo_set_otg_detect(0);
	}
}
#endif
//end to add by weijun for dock_pin charge task-10658685
...
//start to add by weijun for dock_pin charge task-10658685
#if defined(CONFIG_TCT_POGO)
	INIT_DELAYED_WORK(&chg->dock_otg_work, smblib_dock_otg_work);
#endif
//end to add by weijun for dock_pin charge task-10658685
...
#if defined(CONFIG_TCT_CHG_COMMON)
#if defined(CONFIG_TCT_HOLI_BREMENTF) || defined(CONFIG_TCT_HOLI_BREMENVISIBLE)
	INIT_DELAYED_WORK(&chg->adapter_type_work, smblib_adapter_type_work);
#endif
#endif
/* End added by bitao.xiong for task-10699316 on 2021-02-03 */
...
#if defined(CONFIG_TCT_POGO)
		cancel_delayed_work_sync(&chg->dock_otg_work);
#endif
