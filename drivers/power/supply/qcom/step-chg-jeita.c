...
//task:11022749 add by wentaohe@tcl.com
#ifdef CONFIG_TCT_DEBUG_KLOG
#include <linux/klog.h>
#endif
//task:11022749 add end
...
//Add-begin hysteresis to 50C recharging
#if defined(CONFIG_TCT_CHG_COMMON)
#define HI_TEMP_HYS_VOTER "HI_TEMP_HYS_VOTER"
#define HI_TEMP_HYS_LIMIT_LOWER_TEMP_LVL (500)
#define HI_TEMP_HYS_LIMIT_UPPER_TEMP_LVL (550)
#define LOW_TEMP_HYS_VOTER "LOW_TEMP_HYS_VOTER"

#if defined(CONFIG_TCT_IEEE1725)
#define LOW_TEMP_HYS_LIMIT_LOWER_TEMP_LVL (30)
#define LOW_TEMP_HYS_LIMIT_UPPER_TEMP_LVL (50)
#else
#define LOW_TEMP_HYS_LIMIT_LOWER_TEMP_LVL (10)
#define LOW_TEMP_HYS_LIMIT_UPPER_TEMP_LVL (20)
#endif

static int handle_temp_hys_limit(struct step_chg_info *chip)
{
	union power_supply_propval pval = {0, };
	int rc = 0;

	struct votable *chg_disable_votable = find_votable("CHG_DISABLE");
	...
	if (rc < 0) {
		...
		vote(chg_disable_votable, HI_TEMP_HYS_VOTER, false, 0);
		vote(chg_disable_votable, LOW_TEMP_HYS_VOTER, false, 0);
		return rc;
	}
        // for high temperature
	if (pval.intval >= HI_TEMP_HYS_LIMIT_UPPER_TEMP_LVL) {
		vote(chg_disable_votable, HI_TEMP_HYS_VOTER, true, 0);
		pr_debug("[High] btemp %d>=%d, disable charge\n",
				pval.intval, HI_TEMP_HYS_LIMIT_UPPER_TEMP_LVL);

//task:11022749 add by wentaohe@tcl.com
#ifdef CONFIG_TCT_DEBUG_KLOG
		klog_warn("key=BAT_NO_CHG|event=Hight temp over 55\n");
#endif
//task:11022749 add end
	} else if (pval.intval <= HI_TEMP_HYS_LIMIT_LOWER_TEMP_LVL) {
		vote(chg_disable_votable, HI_TEMP_HYS_VOTER, false, 0);
		pr_debug("[High] btemp %d<=%d, enable charge\n",
				pval.intval, HI_TEMP_HYS_LIMIT_LOWER_TEMP_LVL);
	}

	// for low temperature
	if (pval.intval <= LOW_TEMP_HYS_LIMIT_LOWER_TEMP_LVL) {
		vote(chg_disable_votable, LOW_TEMP_HYS_VOTER, true, 0);
		pr_debug("[LOW] btemp %d<=%d, disable charge\n",
				pval.intval, LOW_TEMP_HYS_LIMIT_LOWER_TEMP_LVL);
//task:11022749 add by wentaohe@tcl.com
#ifdef CONFIG_TCT_DEBUG_KLOG
		klog_warn("key=BAT_NO_CHG|event=Low temp\n");
#endif
//task:11022749 add end
	} else if (pval.intval > LOW_TEMP_HYS_LIMIT_UPPER_TEMP_LVL) {
		vote(chg_disable_votable, LOW_TEMP_HYS_VOTER, false, 0);
		pr_debug("[LOW] btemp %d>%d, enable charge\n",
				pval.intval, LOW_TEMP_HYS_LIMIT_UPPER_TEMP_LVL);
	}
	return 0;
}
#endif
// Add-end hysteresis to 50C recharging
...
// Add-begin  hysteresis to 50C or 2C recharging
#if defined(CONFIG_TCT_CHG_COMMON)
	handle_temp_hys_limit(chip);
#endif
// Add-end hysteresis to 50C or 2C recharging
