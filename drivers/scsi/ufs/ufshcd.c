...
// for io boost
#define REQ_UI (1 << 31)
#define rq_is_ui(rq) (!!((rq)->cmd_flags & REQ_UI))
#define rq_is_read(rq) (req_op(rq) == REQ_OP_READ)

static inline bool use_hoq(struct scsi_cmnd *cmd)
{
	struct request *rq;
	...
	if (!cmd || !cmd->request)
		return false;
	rq = cmd->request;
	return rq_is_ui(rq) && rq_is_read(rq);
}
#endif
