...
int headset_power_flag; //wanying.chen add power flag PR 10976821
struct fsa4480_priv {
		...
		u32 switch_control, u32 switch_enable)
{
	...
	dev_dbg(fsa_priv->dev,"%s &prev_control:%d  &prev_enable:%d \n",__func__,prev_control,prev_enable);
	...
	dev_dbg(fsa_priv->dev," %s write reg switch ok  switch_control:%d  switch_enable:%d \n",__func__,switch_control,switch_enable);
}
						...
						struct fsa4480_priv *fsa_priv)
{
	...
	case TYPEC_ACCESSORY_AUDIO:
		/* activate switches */
		//wanying.chen add power flag PR 10976821
		dev_dbg(dev, "%s:  headset_power_flag= %d\n", __func__, headset_power_flag);
		if (headset_power_flag ==1){
	...
	case TYPEC_ACCESSORY_NONE:
		...
		headset_power_flag = 0; //wanying.chen add power flag PR 10976821
		dev_dbg(dev, "%s:  set headset_power_flag= %d\n", __func__, headset_power_flag);
		/* deactivate switches */
			 ...
			 enum fsa_function event)
{
	...
	case FSA_MIC_GND_SWAP:
		regmap_read(fsa_priv->regmap, FSA4480_SWITCH_CONTROL,
				&fsa_priv->switch_control);
		dev_dbg(fsa_priv->dev,"%s fsa_priv->switch_control:%d\n",__func__,fsa_priv->switch_control);
		if ((fsa_priv->switch_control & 0x07) == 0x07)
			fsa_priv->switch_control = 0x0;
		else
			{
				fsa_priv->switch_control = 0x7;
				headset_power_flag = 1;  //wanying.chen add power flag PR 10976821
				dev_dbg(fsa_priv->dev, "%s: set headset_power_flag= %d\n", __func__, headset_power_flag);
			}
