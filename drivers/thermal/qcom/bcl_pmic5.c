...
{
    /*start to add by weijun for modify vbat threshold to 2300mV*/
    #if !IS_ENABLED(CONFIG_GKI_TCT_CHG_COMMON)
    #if IS_ENABLED(CONFIG_TCT_HOLI_TRANSFORMERVZW)
    //bcl_write_register(bcl_perph, BCL_MONITOR_EN, 0); // disable bcl
    bcl_write_register(bcl_perph, BCL_VBAT_COMP_TLOW, 0x02);
    #endif
    #endif
    /*end to add by weijun for modify vbat threshold to 2300mV*/
