				 ...
				 struct fuse_copy_state *cs, size_t nbytes)
{
	...
	if (fuse_passthrough_workaround_enabled(fc) &&
		...
		index = req->args->out_numargs - 1;
		WARN_ON(index != 0 && index != 1);
		WARN_ON(req->args->out_args[index].size != sizeof(*out));
		out = req->args->out_args[index].value;
		flags = out->open_flags;
		if ((flags & FOPEN_DIRECT_IO) || !(flags & FOPEN_KEEP_CACHE)) {
			pr_debug("FUSE: disable passthrough, #flags %x\n", flags);
		} else if (get_user(fd, (u32 __user *)out->fh)) { // TODO: valid fd
			pr_err("FUSE: get_user fail, disable passthrough mode\n");
			fuse_passthrough_enable = 0;
		} else if (fd >= 0) {
			struct fuse_passthrough_out pto;
			pto.fd = fd;
			pto.len = 0;
			out->passthrough_fh = fuse_passthrough_open(fud, &pto);
		}
	}
#endif
