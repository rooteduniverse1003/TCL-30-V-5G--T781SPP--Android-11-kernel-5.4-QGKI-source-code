...
#ifdef CONFIG_TCT_UI_TURBO
static inline void uiturbo_dynamic_enqueue_futex(u32 __user *uaddr,
						 struct futex_q *q)
{
	u32 uval;
	pid_t pid;
	...
	if (same_thread_group(current, p) && !test_task_uiturbo(p)) {
		q->ui_dep_task = p;
...
static inline void uiturbo_dynamic_dequeue_futex(struct futex_q *q)
{
...
static void __unqueue_futex(struct futex_q *q)
{
	...
	hb_waiters_dec(hb);
#ifdef CONFIG_TCT_UI_TURBO
	uiturbo_dynamic_dequeue_futex(q);
#endif
}
...
#ifdef CONFIG_TCT_UI_TURBO
	/* we expect futex owner_tid store at @uaddr + 1 */
	uiturbo_dynamic_enqueue_futex(uaddr + 1, &q);
#endif
