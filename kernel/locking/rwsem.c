...
#ifdef CONFIG_TCT_UI_TURBO
static inline void uiturbo_rwsem_list_add(struct list_head *entry,
...
static inline void uiturbo_dynamic_enqueue_rwsem(struct rw_semaphore *sem)
{
	struct task_struct *owner;
	unsigned long flags;
	...
	if (likely(!test_set_dynamic_uiturbo(current)))
		return;
	if (unlikely(sem->ui_dep_task))
		return;

	owner = rwsem_owner_flags(sem, &flags);
	if (!(flags & RWSEM_READER_OWNED) && owner && !test_task_uiturbo(owner)) {
...
static inline void uiturbo_dynamic_dequeue_rwsem(struct rw_semaphore *sem)
{
	if (unlikely(sem->ui_dep_task == current)) {
		dynamic_uiturbo_dequeue(current, DYNAMIC_UITURBO_RWSEM);
		sem->ui_dep_task = NULL;
	}
}
#endif
...
#else
	if (!already_on_list)
		uiturbo_rwsem_list_add(&waiter.list, &sem->wait_list);
#endif
...
#ifdef CONFIG_TCT_UI_TURBO
	uiturbo_dynamic_enqueue_rwsem(sem);
#endif
...
#ifdef CONFIG_TCT_UI_TURBO
	uiturbo_dynamic_dequeue_rwsem(sem);
#endif
