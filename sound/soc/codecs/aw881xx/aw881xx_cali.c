...
#define AWINIC_CALI_FILE  "/oempersist/audio/aw881xx_cali"
#define AW_INT_DEC_DIGIT 10
#if !IS_ENABLED(CONFIG_TCT_GKI_TRANSFOMERVZW)
static int aw881xx_write_cali_re_to_file(uint32_t cali_re, uint8_t channel)
{
	...
	snprintf(buf, PAGE_SIZE, "%10u", cali_re);
				...
				const char *buf, size_t count)
{
	...
	cali_attr->cali_re = databuf[0] * (1 << AW881XX_DSP_RE_SHIFT) / 1000;
 	...
 	pr_info("%s : re = %d \n ",__func__,cali_attr->re);//wanying.chen add 2021.1.28
