...
static void aw882xx_software_reset(struct aw882xx *aw882xx)
{
	aw_dev_info(aw882xx->dev, "%s: enter\n", __func__);

	aw882xx_i2c_write(aw882xx,
			AW882XX_SOFTWARE_RST_REG, AW882XX_SOFTWARE_RST_VALUE);
}
...
static void aw882xx_reg_loaded(const struct firmware *cont, void *context)
{
	...
	if (aw882xx->reg_cont == NULL) {
		aw882xx->reg_cont = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	} else if (aw882xx->reg_cont->len < cont->size) {
		kfree(aw882xx->reg_cont);
		aw882xx->reg_cont = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	}
	if (!aw882xx->reg_cont) {
	...
	memcpy(aw882xx->reg_cont->data, cont->data, cont->size);
	aw882xx->reg_cont->len = cont->size;
	release_firmware(cont);
	...
	if (aw882xx->sync_start)
		aw882xx_software_reset(aw882xx);

	ret = aw882xx_reg_container_update(aw882xx, aw882xx->reg_cont);
	if (ret < 0) {
...
static int aw882xx_dev_start(struct aw882xx *aw882xx)
{
	aw_dev_info(aw882xx->dev, "%s: enter\n", __func__);

	if ((aw882xx->reg_cont != NULL) && (aw882xx->sync_start)) {
		aw882xx_software_reset(aw882xx);
		aw882xx_reg_container_update(aw882xx, aw882xx->reg_cont);
	}
...
static void aw882xx_parse_sync_dt(struct aw882xx *aw882xx,
	struct device_node *np)
{
	int ret;

	ret = of_property_read_u32(np, "sync-flag", &aw882xx->sync_start);
	if (ret < 0) {
		aw_dev_info(aw882xx->dev,
			"%s: read sync flag failed,sync off\n", __func__);
		aw882xx->sync_start = AW881XX_SYNC_OFF;

	} else {
		aw_dev_info(aw882xx->dev,
			"%s: sync flag is %d\n", __func__, aw882xx->sync_start);
	}
}
		...
		struct device_node *np)
{
	...
	aw882xx_parse_sync_dt(aw882xx, np);
}
				...
				const char *buf, size_t count)
{
	...
	flag = ((flag == AW881XX_SYNC_OFF) ? AW881XX_SYNC_OFF : AW881XX_SYNC_ON);

	if (aw882xx->sync_start == flag) {
		aw_dev_info(aw882xx->dev, "%s: sync status [%d] does not change\n",
					__func__, aw882xx->sync_start);
		return count;
	}

	aw_dev_info(aw882xx->dev, "%s :set phase sync status : [%d]\n",
					__func__, flag);

	aw882xx->sync_start = flag;
