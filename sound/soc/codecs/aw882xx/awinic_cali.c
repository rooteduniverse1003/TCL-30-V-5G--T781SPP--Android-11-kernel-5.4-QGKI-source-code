...
static struct aw_cali_data re_cali_temp[2];
...
static void aw882xx_cali_store_re(struct aw882xx *aw882xx)
{
    int ret = 0;
    struct aw882xx_chan_info *chan_info = &aw882xx->chan_info;
    int32_t cali_re,data_mohm;


    aw_dev_info(aw882xx->dev, "%s:channel %d re_data : %d , count %d\n", __func__,
                    chan_info->channel,  re_cali_temp[chan_info->channel].re_data, 
                    re_cali_temp[chan_info->channel].count);

    cali_re = re_cali_temp[chan_info->channel].re_data/re_cali_temp[chan_info->channel].count;
    data_mohm = FIXED_RE_TO_MOHM(cali_re);
    aw_dev_info(aw882xx->dev, "%s: data : %d , data_mohm %d\n", __func__,  cali_re, data_mohm);
    ...
    ret = aw_write_data_to_dsp(INDEX_PARAMS_ID_RX_RE, (void *)&cali_re,
                sizeof(int32_t), chan_info->channel);
    if (ret < 0) {
        aw_dev_err(aw882xx->dev, "%s: dsp_msg_write error: %d\n",
            __func__, INDEX_PARAMS_ID_RX_RE);
    }
            ...
            unsigned int cmd, unsigned long arg)
{
    ...
    case AW882XX_IOCTL_ENABLE_CALI:
        ...
        if(!g_cali_status){
            aw882xx_cali_store_re(aw882xx);
        }
        else{
            memset(&re_cali_temp[chan_info->channel],0,sizeof(struct aw_cali_data));
        }
    ...
    case AW882XX_IOCTL_GET_VMAX:
        ...
        aw_dev_info(aw882xx->dev, "%s: data : %d , data_len %d\n", __func__,  *((int32_t *)data_ptr), data_len);
        if(index == INDEX_PARAMS_ID_RX_REAL_DATA)
        {
            re_cali_temp[chan_info->channel].re_data += *((int32_t *)data_ptr);
            re_cali_temp[chan_info->channel].count++;
        }
