...
static int aw_check_monitor_profile_v_0_1_1(struct aw882xx *aw882xx,
					struct aw882xx_container *cont)
{
...
static void aw_parse_monitor_hdr_v_0_1_1(struct aw882xx *aw882xx,
					struct aw882xx_container *cont)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
			(struct aw_monitor_hdr_v_0_1_1 *)cont->data;
	...
	aw_dev_info(aw882xx->dev, "%s:monitor_switch:%d, monitor_time:%d (ms), monitor_count:%d\n",
		__func__, monitor_cfg->monitor_switch, monitor_cfg->monitor_time,
		monitor_cfg->monitor_count);

	aw_dev_info(aw882xx->dev, "%s: logic_switch:%d,  ipeak_switch:%d, gain_switch:%d, vmax_switch:%d\n",
		__func__, monitor_cfg->logic_switch, monitor_cfg->ipeak_switch, monitor_cfg->gain_switch,
		monitor_cfg->vmax_switch);

	aw_dev_info(aw882xx->dev, "%s:temp_switch:%d, temp_aplha:%d, vol_switch:%d, vol_aplha:%d\n",
...
static int aw_parse_temp_data_v_0_1_1(struct aw882xx *aw882xx,
			struct aw882xx_container *cont)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
			(struct aw_monitor_hdr_v_0_1_1 *)cont->data;
...
static int aw_parse_vol_data_v_0_1_1(struct aw882xx *aw882xx,
			struct aw882xx_container *cont)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
			(struct aw_monitor_hdr_v_0_1_1 *)cont->data;
...
static int aw_parse_monitor_data_v_0_1_1(struct aw882xx *aw882xx,
					struct aw882xx_container *cont)
{
	int ret;
	struct aw_monitor_cfg *monitor_cfg = aw882xx->monitor.monitor_cfg;

	ret = aw_check_monitor_profile_v_0_1_1(aw882xx, cont);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "%s:check %s failed\n",
				__func__, AW_MONITOR_FILE);
		return ret;
	...
	aw_parse_monitor_hdr_v_0_1_1(aw882xx, cont);

	ret = aw_parse_temp_data_v_0_1_1(aw882xx, cont);
	if (ret < 0)
		return ret;

	ret = aw_parse_vol_data_v_0_1_1(aw882xx, cont);
	if (ret < 0) {
		if (monitor_cfg->temp_info.aw_table != NULL) {
			kfree(monitor_cfg->temp_info.aw_table);
			monitor_cfg->temp_info.aw_table = NULL;
					...
					struct aw882xx_container *cont)
{
	...
	if (cont->len < sizeof(struct aw_monitor_hdr)) {
		aw_dev_err(aw882xx->dev, "%s:data size smaller than hdr , please check monitor bin\n",
					__func__);
		return -ENOMEM;
	}
					...
					struct aw882xx_container *cont)
{
	...
	case AW_MONITOR_HDR_VER_0_1_1:
		return aw_parse_monitor_data_v_0_1_1(aw882xx, cont);
	default:
